...
public class ServerAutoSetup extends Module {
    ...
    public ServerAutoSetup() {
        super(Eclipse.UTILITY, "server-auto-setup", "Applies a Karasique Meteor module profile without touching render, themes, or binds.");
    }
    ...
    public void onActivate() {
        ...
        if (disableRiskyCombat.get()) {
            disabled += disable("crystal-killer", "bed-aura", "anchor-aura", "auto-crystal", "auto-city", "auto-trap", "auto-web", "surround", "self-trap", "burrow");
        }

        if (enableSafeModules.get()) {
            enabled += enable("sprint", "killer", "auto-totem", "auto-weapon", "auto-tool", "inventory-tweaks", "anti-packet-kick");
    ...
    private int configureKarasiqueBaseline() {
        ...
        changed += set("killer", "rotation", "OnAttack");
        changed += set("killer", "auto-switch", "false");
        changed += set("killer", "only-weapons", "true");
        changed += set("killer", "only-on-click", "false");
        changed += set("killer", "pause-on-use", "true");
        changed += set("killer", "priority", "ClosestAngle");
        changed += set("killer", "max-targets", "1");
        changed += set("killer", "range", "4.0");
        changed += set("killer", "walls-range", "2.6");
        changed += set("killer", "ignore-named", "false");
        changed += set("killer", "custom-delay", "false");
        changed += set("killer", "hit-delay", "11");
        ...
        changed += set("litematica-printer", "blocks-per-tick", "3");
        changed += set("litematica-printer", "horizontal-range", "5");
        changed += set("litematica-printer", "vertical-range", "5");
        changed += set("litematica-printer", "scan-limit", "2200");
        changed += set("litematica-printer", "tick-delay", "1");
        ...
        changed += set("litematica-printer", "move-cooldown", "2");
        changed += set("litematica-printer", "correction-pause", "60");
        changed += set("litematica-printer", "retry-delay", "5");
