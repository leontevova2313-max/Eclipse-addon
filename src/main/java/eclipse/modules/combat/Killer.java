...
public class Killer extends Module {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        targets.clear();
        TargetUtils.getList(targets, this::isValidTarget, priority.get(), maxTargets.get());
    ...
    private void attack(LivingEntity target) {
        FindItemResult weapon = InvUtils.findInHotbar(this::isWeapon);
        boolean shouldSwap = autoSwitch.get() && weapon.found() && !weapon.isMainHand();
        if (onlyWeapons.get() && !isWeapon(mc.player.getMainHandStack()) && !shouldSwap) return;

        Runnable attack = () -> {
            boolean swapped = false;
            if (shouldSwap) swapped = InvUtils.swap(weapon.slot(), false);
