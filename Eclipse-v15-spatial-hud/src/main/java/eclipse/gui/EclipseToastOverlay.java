...
public final class EclipseToastOverlay {
    ...
    private static LivingEntity entityIcon;
    ...
    public static void show(String title, String body, ItemStack icon, int accent) {
        ...
        EclipseToastOverlay.entityIcon = null;
        EclipseToastOverlay.accent = accent;
        EclipseToastOverlay.startedAt = System.currentTimeMillis();
    }

    public static void show(String title, String body, LivingEntity entity, int accent) {
        EclipseToastOverlay.title = title;
        EclipseToastOverlay.body = body;
        EclipseToastOverlay.icon = ItemStack.EMPTY;
        EclipseToastOverlay.entityIcon = entity;
        EclipseToastOverlay.accent = accent;
        EclipseToastOverlay.startedAt = System.currentTimeMillis();
    }
    ...
    public static void render(DrawContext context) {
        ...
        boolean hasEntityIcon = entityIcon != null && entityIcon.isAlive();
        boolean hasIcon = !icon.isEmpty() || hasEntityIcon;
        ...
        if (hasEntityIcon) {
            int x1 = x + 8;
            int y1 = y + 4;
            InventoryScreen.drawEntity(context, x1, y1, x1 + 22, y1 + 26, 18, 0.0F, x1 + 11, y1 + 10, entityIcon);
            textX += 24;
        } else if (hasIcon) {
