...
public class LitematicaPrinter extends Module {
    ...
    private final Setting<Boolean> grimSafe = sgSafety.add(new BoolSetting.Builder()
        .name("grim-safe")
        .description("Uses conservative pacing and pauses while movement/corrections make Grim likely to setback.")
        .defaultValue(true)
        .build()
    );

    private final Setting<Double> maxTickMs = sgSafety.add(new DoubleSetting.Builder()
        .name("max-tick-ms")
        .description("Maximum main-thread time spent scanning and placing per tick.")
        .defaultValue(2.5)
        .range(0.5, 12.0)
        .sliderRange(1.0, 6.0)
        .decimalPlaces(1)
    ...
    private final Setting<Integer> progressInterval = sgRender.add(new IntSetting.Builder()
        .name("progress-interval")
        .description("Ticks between progress scan slices.")
        .defaultValue(5)
        .range(1, 40)
        .sliderRange(1, 20)
        .build()
    ...
    private int correctionBurst;
    private int correctionBurstTicks;
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        if (event.packet instanceof PlayerPositionLookS2CPacket && pauseOnCorrection.get()) {
            if (correctionBurstTicks > 0) correctionBurst++;
            else correctionBurst = 1;
            correctionBurstTicks = 100;
            correctionTicks = correctionPause.get() + (grimSafe.get() ? Math.min(160, correctionBurst * 20) : 0);
            pendingAttempts.clear();
            renderCandidates.clear();
        }
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        tickCooldowns();
        if (correctionBurstTicks > 0) correctionBurstTicks--;
        else correctionBurst = 0;
        if (tickSequence % Math.max(1, progressInterval.get()) == 0) updateProgressSnapshot(deadline);
        ...
        List<Candidate> candidates = collectCandidates(deadline);
    ...
    private boolean canRunThisTick() {
        ...
        if (grimSafe.get() && !isPlayerStableForGrim()) {
            delayTicks = Math.max(delayTicks, 2);
            return false;
        }
    ...
    private boolean isPlayerStableForGrim() {
        Vec3d velocity = mc.player.getVelocity();
        double horizontalSpeed = Math.sqrt(velocity.x * velocity.x + velocity.z * velocity.z);
        if (horizontalSpeed > 0.30 || Math.abs(velocity.y) > 0.18) return false;
        if (mc.player.fallDistance > 0.35F || mc.player.isGliding()) return false;
        return mc.player.isOnGround() || mc.player.isClimbing() || mc.player.isTouchingWater();
    }

    private long tickDeadline() {
        long nanos = (long) Math.max(500_000.0, maxTickMs.get() * 1_000_000.0);
        return System.nanoTime() + nanos;
    }

    private boolean pastDeadline(long deadline) {
        return System.nanoTime() >= deadline;
    }

    private List<Candidate> collectCandidates(long deadline) {
        ...
        if (selected != null && !selected.boxes().isEmpty()) {
            collectSelectedCandidates(candidates, selected, origin, centerY, hRange, vRange, eye, maxDistanceSq, deadline);
    ...
    private void collectSelectedCandidates(List<Candidate> candidates, SelectedPlacement selected, BlockPos origin, int centerY, int hRange, int vRange, Vec3d eye, double maxDistanceSq, long deadline) {
    ...
    private void updateProgressSnapshot(long deadline) {
        ...
        progressSnapshot = progressTracker.scan(selected, progressScanPerTick.get(), deadline);
    }
    ...
    private boolean needsWrongStateReplacement(BlockState worldState, BlockState schematicState) {
        if (!replaceWrongState.get()) return false;
        if (worldState.getBlock() != schematicState.getBlock()) return false;
        if (worldState.equals(schematicState)) return false;

        if (exactState.get()) return true;
        return importantState.get() && !importantPropertiesMatch(worldState, schematicState);
    }
    ...
    private FindItemResult findHotbarOrMove(Predicate<ItemStack> predicate) {
        ...
        int target = chooseHotbarMoveTarget(predicate);
        InvUtils.move().from(inventory.slot()).toHotbar(target);
        moveCooldown = effectiveMoveCooldown();
        return InvUtils.findInHotbar(predicate);
    }

    private int chooseHotbarMoveTarget(Predicate<ItemStack> predicate) {
        FindItemResult emptyHotbar = InvUtils.findInHotbar(ItemStack::isEmpty);
        if (emptyHotbar.found() && emptyHotbar.isHotbar()) return emptyHotbar.slot();
        ...
        for (int slot = 0; slot < 9; slot++) {
            ...
            if (predicate.test(stack)) return slot;
    ...
    private int effectiveBlocksPerTick() {
        ...
        if (grimSafe.get()) {
            if (correctionTicks > 0 || currentPing() > MID_LATENCY_MS) return 1;
            return clamp(configured, 1, 2);
        }
    ...
    private int effectiveTickDelay() {
        ...
        if (grimSafe.get()) return Math.max(1, configured);
    ...
    private int effectiveRetryDelay() {
        ...
        if (grimSafe.get()) return Math.max(configured, currentPing() > MID_LATENCY_MS ? 10 : 6);
    ...
    private int effectiveMoveCooldown() {
        ...
        if (grimSafe.get()) return Math.max(configured, 2);
    ...
    private final class ProgressTracker {
        ...
        private ProgressSnapshot scan(SelectedPlacement selected, int limit, long deadline) {
            if (identity != selected.identity()) start(selected);
            if (boxes.isEmpty()) return ProgressSnapshot.empty();

            int scannedThisTick = 0;
            while (scannedThisTick < limit && boxIndex < boxes.size() && !pastDeadline(deadline)) {
